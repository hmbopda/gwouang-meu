-- Fix: subquery uses ungrouped column "pc1.parent_id" from outer query
-- Use MIN(pc1.parent_id) instead since in the ELSE branch there is exactly 1 shared parent
        ...
        CASE COUNT(DISTINCT pc1.parent_id)
            ...
            ELSE (
                ...
                WHERE pr.child_id = v_child_id
                  AND pr.parent_id = MIN(pc1.parent_id)
                LIMIT 1
            )::sibling_type_enum
        END,
